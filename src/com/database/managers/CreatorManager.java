...
public class CreatorManager {
	...
	private static void createResearches(int n) {
		Random r = new Random();
		for (int i = 0; i < n; i++) {
	...
	private static void createInvestigators(int n) {
		Random r = new Random();
		for (int j = 0; j < n; j++) {
			long users = JpaUtil.executeQueryAndGetSingle("Select count(*) from User u", Long.class);
			Investigator re = ResearchManager.createInvestigator(r.nextInt((int)users), "ISCTE-IUL");
			if(re==null)
				j--;
		}
