...
public class AdminEventsBean implements Serializable{
	...
	public void init() {
		...
		pendingEvents = JpaUtil.executeQuery("Select count(*) from Event p where p.state = "+ State.ON_APPROVAL.ordinal(), Long.class) .get(0);
		eventsDeveloping= JpaUtil.executeQuery("Select count(*) from Event p where p.state = "+ State.DEVELOPING.ordinal(), Long.class) .get(0);
	...
	private double getLikesPerMonth() {
		...
		double allViews = JpaUtil.executeQuery("Select count(*) from Event p join p.likes l", Long.class).get(0);
	...
	private double getViewsPerMonth() {
		...
		LocalDateTime now = LocalDateTime.now();
		LocalDateTime date = begin.minusYears(now.getYear());
		double allViews = JpaUtil.executeQuery("Select count(*) from Event p join p.views v", Long.class).get(0);
		double total = date.getYear() * 12 + date.getMonthValue();
		return allViews/total;
	}
	 ...
	 */
	public List<Long> getLastMonthsEventsCreated(){
		LocalDateTime later = LocalDateTime.now().minusMonths(lastMonths-1);
		List<Long> months = new ArrayList<>();
		for (int i = 0; i < lastMonths ; i++) {
			months.add(JpaUtil.executeQuery("select count(p) from Event p WHERE YEAR(p.creationDate) = "+later.getYear()+" AND MONTH(p.creationDate) = "+later.getMonthValue(), Long.class).get(0));
			later = later.plusMonths(1);
		}
		return months;
	}
	...
	public List<Long> getLastMonthsViews(){
		...
		List<Long> months = new ArrayList<>();
		for (int i = 0; i < lastMonths ; i++) {
			months.add(JpaUtil.executeQuery("Select count(*) from Event p join p.views v where YEAR(v.date) = "+ later.getYear() +"and MONTH(v.date) = "+later.getMonthValue(), Long.class).get(0));
			later = later.plusMonths(1);
		}
		return months;
	}
	...
	public List<Long> getLastMonthsLikes(){
		...
		List<Long> months = new ArrayList<>();
		for (int i = 0; i < lastMonths ; i++) {
			months.add(JpaUtil.executeQuery("Select count(*) from Event p join p.likes v where YEAR(v.date) = "+ later.getYear() +"and MONTH(v.date) = "+later.getMonthValue(), Long.class).get(0));
			later = later.plusMonths(1);
		}
		return months;
	}
	...
	public void removeProject(ActionEvent e ) {
		System.out.println(e.getComponent().getAttributes().get("eventId"));
	}
